...
def init_routes(density):
    ...
    def get_od(node1, node2, k, lane=0):
        ...
        # 只在特定区域创建初始车辆
        target_nodes = ['nt13', 'nt14', 'nt18', 'nt19']
        node_pair_str = node1 + "_" + node2

        # 默认车辆数
        local_car_num = car_num
        ...
        if (node1 in target_nodes or node2 in target_nodes):
            ...
            # 非目标区域减少车辆数
            local_car_num = int(car_num * 0.3)

        return init_flow % (str(k), source_edge, sink_edge, lane, local_car_num)
...
def output_flows(peak_flow1, peak_flow2, density, seed=None):
    ...
    str_flows = '<routes>\n'
    # 修改车辆类型定义，保持合理的参数
    str_flows += '  <vType id="type1" length="5" accel="3.5" decel="7.5" sigma="0.5" minGap="2.0" maxSpeed="12"/>\n'

    # 降低初始密度
    if density > 0:
        str_flows += init_routes(density * 0.5)  # 减小初始密度
    ...
    # 创建更合理的流量比例
    ratios1 = np.array([0.3, 0.5, 0.7, 0.5, 0.4, 0.3, 0.2])  # 降低整体流量
    ratios2 = np.array([0.3, 0.5, 0.7, 0.5, 0.4, 0.3, 0.2])

    # 使用更适中的流量
    flows1 = peak_flow1 * 0.4 * ratios1
    flows2 = peak_flow1 * 0.4 * ratios1
    flows3 = peak_flow2 * 0.4 * ratios2
    flows4 = peak_flow2 * 0.4 * ratios2
    ...
    for i in range(len(times) - 1):
        ...
        # 仅在时间点900-1200和时间点1500-1800添加局部高流量
        if i == 3 or i == 5:  # 对应times数组中索引为3和5的时间点
            # 只选择特定区域添加高流量
            target_nodes = ['nt13', 'nt18']  # 只针对两个重点路口

            # 为特定路段创建高流量
            spike_flow = 1800  # 适中的尖峰流量

            # 选择特定的起点和终点
            if i == 3:  # 在900-1200时间段
                special_sources = [srcs[1][0]]  # 选择特定起点
                special_sinks = [sinks[0][0]]  # 选择特定终点
            else:  # 在1500-1800时间段
                special_sources = [srcs[3][0]]  # 选择另一个特定起点
                special_sinks = [sinks[2][0]]  # 选择另一个特定终点

            for src_idx, e1 in enumerate(special_sources):
                for sink_idx, e2 in enumerate(special_sinks):
                    special_name = name + '_special_' + str(src_idx) + '_' + str(sink_idx)
                    str_flows += ext_flow % (special_name, e1, e2, t_begin, t_end, spike_flow)
...
def output_tls(tls, phase):
    ...
    for i in range(1, 26):
        ...
        # 为特定重点路口使用更长的红灯时间
        if i in [13, 18]:  # 为目标节点13和18修改信号灯
            local_duration = [20, 5]  # 缩短绿灯，延长黄灯
        else:
            local_duration = phase_duration

        for k, p in enumerate(three_phases):
            str_adds += phase % (local_duration[k % 2], p)
        str_adds += '  </tlLogic>\n'
    str_adds += '</additional>\n'
    return str_adds
...
def main():
    ...
    # 调整为适中的流量，只在特定区域和时间创建拥堵
    write_file('./exp.rou.xml', output_flows(1500, 1800, 0.3))
