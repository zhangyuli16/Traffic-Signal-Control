...
class TrafficSimulator:
    ...
    def _init_sim(self, seed, gui=False):
        """Initialize SUMO simulation with improved port management
        ...
        while not connected and port_attempts < max_attempts:
            try:
                # Calculate different port for each attempt
                current_port = self.port + port_attempts * 10
                
                command = [checkBinary(app), '-c', sumocfg_file]
                command += ['--seed', str(seed)]
                command += ['--remote-port', str(current_port)]
                ...
                logging.info("Starting SUMO on port {0}".format(current_port))
                sumo_process = subprocess.Popen(command)
                
                # Store the process ID for later cleanup
                self.sumo_process = sumo_process
                
                # Wait a bit before attempting connection
                time.sleep(2)
                
                # Try to connect to SUMO
                self.sim = traci.connect(port=current_port)
                connected = True
                
                # Store the used port for proper cleanup later
                self.current_port = current_port
                logging.info("Connected to SUMO on port {0}".format(current_port))
                
            except Exception as e:
                logging.warning("Failed to connect on port {0}: {1}".format(current_port, str(e)))
                port_attempts += 1
                ...
                # Try to kill the SUMO process if it was started
                try:
        ...
        if not connected:
            raise RuntimeError("Could not connect to SUMO after {0} attempts. Please check if SUMO processes are still running.".format(max_attempts))
    ...
    def terminate(self):
        """Properly close the simulation and clean up resources"""
        try:
            self.sim.close()
            current_port = getattr(self, 'current_port', self.port)
            logging.info("Closed SUMO connection on port {0}".format(current_port))
        except Exception as e:
            logging.warning("Error closing SUMO connection: {0}".format(str(e)))
        
        # Try to kill the SUMO process if it exists
        try:
            if hasattr(self, 'sumo_process') and self.sumo_process:
                self.sumo_process.terminate()
                logging.info("Terminated SUMO process")
        except Exception as e:
            logging.warning("Error terminating SUMO process: {0}".format(str(e)))
        
        # Allow some time for ports to be properly released
        time.sleep(1)
