...
from agents.models import A2C, IA2C, MA2C, IQL, IPPO, SPPO
...
def train(args):
    ...
    if env.agent == 'ia2c':
        ...
        model = MA2C(env.n_s_ls, env.n_a_ls, env.n_w_ls, env.n_f_ls, total_step,
                     config['MODEL_CONFIG'], seed=seed)
    elif env.agent == 'ippo':  # ÃÃÃÃ¶IPPOÃÂ¡ÃÃ®
        model = IPPO(env.n_s_ls, env.n_a_ls, env.n_w_ls, total_step,
                     config['MODEL_CONFIG'], seed=seed)
    elif env.agent == 'sppo':  # æ°å¢SPPOéé¡¹
        model = SPPO(env.n_s_ls, env.n_a_ls, env.n_w_ls, total_step,
                     config['MODEL_CONFIG'], seed=seed)
    elif env.agent == 'iqld':
...
def evaluate_fn(agent_dir, output_dir, seeds, port, demo, policy_type):
    ...
    if agent != 'greedy':
        ...
        elif agent == 'ippo':  # ÃÃÃÃ¶IPPOÃÂ¡ÃÃ®
            model = IPPO(env.n_s_ls, env.n_a_ls, env.n_w_ls, 0, config['MODEL_CONFIG'])
        elif agent == 'sppo':  # æ°å¢SPPOéé¡¹
            model = SPPO(env.n_s_ls, env.n_a_ls, env.n_w_ls, 0, config['MODEL_CONFIG'])
        elif agent == 'iqld':
